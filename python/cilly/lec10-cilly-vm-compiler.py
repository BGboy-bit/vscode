...
def cilly_vm_compiler(ast, code, consts, scopes):
    ...
    def compile_if(node):
        ...
        def compile_while(node):
            _, cond, body = node
        start_addr = get_next_emit_addr()     # while 头地址

        visit(cond)                           # 条件入栈
        jf_addr = emit(JMP_FALSE, -1)         # 条件假 → 跳出循环

        # 记录本层循环，供 break / continue 回填
        loop_info = {"start": start_addr, "breaks": [], "continues": []}
        loop_stack.append(loop_info)

        visit(body)                           # 循环体
        emit(JMP, start_addr)                 # 体跑完回头再判
        end_addr = get_next_emit_addr()       # while 结束位置

        # 补丁：条件为假直接跳到 end_addr
        backpatch(jf_addr, end_addr)
        # break 补丁
        for br in loop_info["breaks"]:
            backpatch(br, end_addr)
        # continue 补丁（回到条件处）
        for ct in loop_info["continues"]:
            backpatch(ct, start_addr)

        loop_stack.pop()

    def compile_break(_node):
        if not loop_stack:
            err('break 不在循环内部')
        j = emit(JMP, -1)
        loop_stack[-1]["breaks"].append(j)

    def compile_continue(_node):
        if not loop_stack:
            err('continue 不在循环内部')
        j = emit(JMP, -1)
        loop_stack[-1]["continues"].append(j)
