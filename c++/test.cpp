...
void InitialJob()
{
   ...
   for (i = 0; i < PNUM; i++)
   {
      ...
      p->curarri = p->arrival;       // 初始动态到达时间为初始时间
...
void ReadytoJob() // 抢占式每个周转时间将所有ready调回job
{
   if(run != NULL) {
      run->state = 'W';
      run->curarri = timenow;
...
void ReadyQueue(char *algo, int t) // 根据作业队列构造就绪队列,algo:算法,t:当前时刻
{
   ...
   {
      if (jcur->curarri <= t) // 如果当前时刻进程已经到达,则将其插入到就绪队列的合适位置
      {
         printf("P%d到达.\n", jcur->pid);
         ...
         else // 就绪队列不为空,遍历就绪队列,将jcur插入到合适位置
         {
            ...
            case 0: // FCFS,RR.根据到达时间arrival查找合适插入点
               while ((rcur != NULL) && (jcur->curarri >= rcur->curarri))
               {
                  rpre = rcur;
                  rcur = rcur->next;
               }
            ...
            case 2: // STRF,根据剩余时间rest查找合适插入点
               while ((rcur != NULL) && (jcur->rest > rcur->rest) || (jcur->rest ==  rcur->rest) && (jcur->curarri > rcur->curarri))
               {
                  rpre = rcur;
                  rcur = rcur->next;
               }
               break;
            case 3: // Priority, Non-Priority,根据优先级和到达时间查找合适插入点
               while ((rcur != NULL) && ((jcur->priority > rcur->priority) || (jcur->priority == rcur->priority) && (jcur->curarri > rcur->curarri)))
               {
                  rpre = rcur;
                  rcur = rcur->next;
               }
